        ...
        <nav className="space-y-2 flex-1">
          {CATEGORIES.map((cat) => (
            <button
              ...
              className={`w-full flex items-center justify-between px-4 py-3 rounded-full text-sm font-semibold transition-all duration-200 ${activeCategory === cat.id
            ...
            >
              ...
              {cat.id !== 'Overview' && getCount(cat.id) > 0 && (
                <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${activeCategory === cat.id ? 'bg-white text-slate-900' : 'bg-slate-100 text-slate-600'
                  }`}>
                  {getCount(cat.id)}
                </span>
              )}
