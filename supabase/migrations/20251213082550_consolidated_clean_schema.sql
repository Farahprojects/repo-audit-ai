...
CREATE TABLE audit_complete_data (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,
...
CREATE INDEX idx_github_accounts_github_id ON github_accounts(github_id);
...
-- System Prompts
CREATE INDEX idx_system_prompts_tier ON system_prompts(tier);

-- Preflights
CREATE INDEX idx_preflights_user_id ON preflights(user_id);
CREATE INDEX idx_preflights_github_account_id ON preflights(github_account_id);
CREATE INDEX idx_preflights_repo_url ON preflights(repo_url);
...
CREATE INDEX idx_audit_jobs_preflight_id ON audit_jobs(preflight_id);
...
CREATE INDEX idx_audit_status_job_id ON audit_status(job_id);

-- Audit Complete Data
CREATE INDEX idx_audit_complete_data_user_id ON audit_complete_data(user_id);
CREATE INDEX idx_audit_complete_data_audit_job_id ON audit_complete_data(audit_job_id);
CREATE INDEX idx_audit_complete_data_repo_url ON audit_complete_data(repo_url);
CREATE INDEX idx_audit_complete_data_created_at ON audit_complete_data(created_at);
...
-- Repos: Users can see repos from their preflights, service role can see all
