...
// Generate complexity fingerprint for cost estimation
async function generateComplexityFingerprint(tree: any[], langData: any, repoData: any, headers: any, owner: string, repo: string, defaultBranch: string) {
  // Basic metrics
...
serve(async (req) => {
  ...
  try {
    ...
    // =========================================================================
    // ACTION: fingerprint - Generate complexity fingerprint for cost estimation
    // =========================================================================
    if (action === 'fingerprint') {
      console.log(`🔍 [github-proxy] Generating fingerprint for: ${owner}/${repo}`);

      // First get basic repo data
      const repoRes = await fetch(
        `https://api.github.com/repos/${owner}/${repo}`,
        { headers }
      );
      ...
      if (!langRes.ok) {
        return new Response(
          JSON.stringify({ error: `Failed to fetch repository languages: ${langRes.status}` }),
          { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }
      ...
      // Get repository tree for file analysis
      ...
      if (!treeResponse.ok) {
        return new Response(
          JSON.stringify({ error: `Failed to fetch repository tree: ${treeResponse.status}` }),
          { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }
      ...
      // Analyze file tree to generate complexity fingerprint
      const fingerprint = await generateComplexityFingerprint(treeData.tree, langData, repoData, headers, owner, repo, defaultBranch);
